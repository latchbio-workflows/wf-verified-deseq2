...
tryCatch(
  {
    ...
    # Filter out samples marked with "__ignore"
    coldata <- coldata %>%
      filter(!.data[[setdiff(colnames(coldata), sample_id_column)[1]]] == "__ignore")

    samples <- make.names(coldata[[sample_id_column]])
    # make sample column values safe
    coldata[[sample_id_column]] <- samples

    # Convert to factor after filtering
    coldata <- coldata %>%
      mutate(across(!all_of(sample_id_column), as.factor))

    p("Condition levels after filtering:")
    print(levels(coldata[[setdiff(colnames(coldata), sample_id_column)[1]]]))
...
write.csv(coldata, file = op("Data/Sample Table.csv"), row.names = FALSE)
...
tryCatch(
  {
    ...
    for (name in names) {
      tryCatch(
        {
          pca_data <- plotPCA(vsd, intgroup = name, returnData = TRUE)
          write.csv(pca_data, file = op("Data/Sample PCA.csv"), row.names = FALSE)
